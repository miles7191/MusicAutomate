...
public class ExponentialFade extends Fade{
	...
	private final int expo = 25;

	public ExponentialFade(double length) {
		super(length);
	}
	...
	public void update(AutoMusic music, Direction direction, double remaining) {
		...
		if(direction == Direction.OUT) {
			music.getTinyMusic().setVolume((Math.pow(expo, percentage)-1) / (expo -1));
		}else if (direction == Direction.IN){
			music.getTinyMusic().setVolume((Math.pow(expo, 1-percentage)-1) / (expo -1));
		}
	}
